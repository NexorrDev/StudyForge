...
export default function NoteEditorPage({ noteId }: { noteId?: string }) {
  ...
  const handleGenerate = async () => {
    ...
    if (!content.trim() && !title.trim()) {
      setGenMsg("Ajoute du contenu à ta fiche d'abord !");
      return;
    }
    setGenerating(true);
    setGenMsg("");
  ...
  return (
    <>
      <style>{`
        ...
        .tiptap-editor-inner p.is-editor-empty:first-child::before { content: attr(data-placeholder); float: left; color: var(--text-muted); pointer-events: none; height: 0; font-style: italic; }
        .tiptap-editor-inner:focus { outline: none; }
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:20, position:"relative", zIndex:1 }} className="animate-slide-up">
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              ...
              <h1 style={{ fontFamily:"var(--font-syne,sans-serif)", fontWeight:800, fontSize:26, color:"var(--text-primary)" }}>
                {noteId ? "Éditer la fiche" : "Nouvelle fiche"}
              </h1>
            </div>
            <div style={{ display:"flex", gap:10 }}>
              <button
                onClick={() => setShowGenPanel(!showGenPanel)}
                style={{ padding:"10px 16px", borderRadius:12, border:"1px solid rgba(124,58,237,0.3)", background:"rgba(124,58,237,0.1)", color:"var(--primary-light)", cursor:"pointer", fontFamily:"var(--font-syne,sans-serif)", fontWeight:600, fontSize:13 }}
              >✦ IA → Flashcards</button>
              <button
                onClick={handleSave}
                disabled={saving || !title.trim()}
                style={{ padding:"10px 22px", borderRadius:12, border:"none", cursor:saving?"not-allowed":"pointer", background:"linear-gradient(135deg,var(--primary),var(--primary-light))", color:"white", fontFamily:"var(--font-syne,sans-serif)", fontWeight:700, fontSize:14, opacity:saving||!title.trim()?0.6:1 }}
              >
          ...
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr", gap:14 }}>
            {[
              ...
              { label:"Tags (virgules)", ph:"ex: quantique, ondes", val:tags, set:setTags },
            ].map(f => (
              <div key={f.label}>
          ...
          {showGenPanel && (
            <div style={{ padding:18, borderRadius:14, background:"rgba(124,58,237,0.06)", border:"1px solid rgba(124,58,237,0.2)" }}>
              <h3 style={{ fontFamily:"var(--font-syne,sans-serif)", fontWeight:700, fontSize:13, marginBottom:12, color:"var(--text-primary)" }}>✦ Générer des flashcards avec l'IA</h3>
          ...
          <div style={{ borderRadius:16, overflow:"hidden", border:"1px solid var(--border)", background:"var(--bg-card)" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:2, padding:"8px 12px", background:"rgba(0,0,0,0.25)", borderBottom:"1px solid var(--border)", flexWrap:"wrap" }}>
              ...
              <div style={{ marginLeft:"auto", display:"flex", gap:2 }}>
                ...
                <ToolBtn onClick={() => editor?.chain().focus().redo().run()} active={false} title="Rétablir (Ctrl+Y)">↪</ToolBtn>
              </div>
            </div>
            ...
            {editor ? (
              ...
              <div style={{ padding:24, color:"var(--text-muted)", fontStyle:"italic" }}>Chargement de l'éditeur...</div>
            )}
          </div>

          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <p style={{ fontSize:12, color:"var(--text-muted)" }}>
              Raccourcis : <kbd style={{ background:"rgba(255,255,255,0.06)", border:"1px solid var(--border)", borderRadius:4, padding:"1px 5px", fontSize:11 }}>Ctrl+B</kbd> gras · <kbd style={{ background:"rgba(255,255,255,0.06)", border:"1px solid var(--border)", borderRadius:4, padding:"1px 5px", fontSize:11 }}>Ctrl+I</kbd> italique · <kbd style={{ background:"rgba(255,255,255,0.06)", border:"1px solid var(--border)", borderRadius:4, padding:"1px 5px", fontSize:11 }}>Ctrl+Z</kbd> annuler
            </p>
